...
func	PrintSignedProposalInfo(stub shim.ChaincodeStubInterface) {
	...
	logger.Info("PrintSignedProposalInfo() executed \n")
	...
	logger.Info(fmt.Sprintln("channelHeader.GetType() => ", common.HeaderType(channelHeader.GetType())))
	logger.Info(fmt.Sprintln("channelHeader.GetChannelId() => ", channelHeader.GetChannelId()))
}

// Prints the creator information
func  PrintCreatorInfo(stub shim.ChaincodeStubInterface) {
	logger.Info(fmt.Sprintln("PrintCreatorInfo() executed "))

	byteData, _ := stub.GetCreator()

	logger.Info(fmt.Sprintln("PrintCreatorInfo => ",string(byteData)))
}